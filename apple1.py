...
class Apple1Py:
    def __init__(self, file="mem.b", size=0x1000):
        ...
        self.optable = {
            ...
            "LDA imm": {
                ...
                "bytes": 0xA9,
                "space": 2,
            },  # LDA immediate (just load hex value into accumulator
            ...
            "BMI rel": {"ascii": "BMI", "bytes": 0x30, "space": 2},
    ...
    def convertToSignedByte(self, byteValue):
        if byteValue > 127:
            return ((byteValue - 127) * -1) + 127
        return byteValue

    def setProgramCounterValueFromRelativeAddress(self, relativeAddressTwoBits):
        if relativeAddressTwoBits <= 127:
            self.pc += relativeAddressTwoBits
        if relativeAddressTwoBits > 127:
            self.pc += relativeAddressTwoBits - (1 << 8)
        self.pc += 1  # because there's a bleedin offset to the previous one

    def addLabelReference(
        self, trueByteValue, bytesGiven, labelsGiven, relativeLabel=False
    ):
        foundLabel = False
        for labelReference in labelsGiven:
            ...
            if (
                False
                and labelReference["name"] == trueByteValue.replace("!", "")
                and labelReference["location"] != False
            ):
                ...
                # okay big disclaimer for new feature
                # commands like BEQ take in relative addresses.
                # so we are passing a relative address if this is true, otherwise
                # we know it's an absolute address
                if self.relativeLabel == True:
                    twoByteAdd = labelReference["location"] - self.pc
                else:
        ...
        if not foundLabel:
            ...
            labelsGiven.append(
                {"name": trueByteValue.replace("!", ""), "location": False}
            )
            if relativeLabel == True:
                # we have to make it a unique identifier
                # so that future relative labels to the same
                # address aren't filled in incorrectly
                # golly gee goober i hate this!!!
                # october 10th sad comment
                # as of this point
                # this hacky solution to labels i have discovered
                # is objectively the best way to handle labels period
                # filling in knowledge
                # so i guess we just default and do this
                # it compiles the same!
                # it just sucks
                bytesGiven.append(
                    "ref!REL!"
                    + str(len(bytesGiven))
                ...
                print(bytesGiven)
                # notice we do not append a pass otherwise we get freaky weaky
                # with the pc being off. remember rel is only one byte
            else:
    ...
    def memoryDisplayer(self):
        ...
        while True:
            ...
            userPrompt = input(
                "(G)oto addr (S)tack (I)nternal Variables (F)lags (B)ig View (C)hange value (Q)uit"
            ).lower()
            if userPrompt == "q":
                break
            if userPrompt == "b":
    ...
    def convert(self, fileIn, debug=False):
        ...
        while True:
            ...
            if trueKeyword == ".string":
                memoryToConvert = (
                    currentByte.strip().replace(".string", "").replace('"', "")[1:]
                )
            ...
            if trueKeyword == "BRK":
                ...
                if trueByteValue[0] == "#":  # immediate
                    ...
                    bytesGiven.append(
                        int(trueByteValue.replace("#", "").replace("$", ""), 16)
                    )
                ...
                if trueByteValue[0] == "$":
                    ...
                    twoByteAdd = int(
                        trueByteValue.replace("#", "").replace("$", ""), 16
                    )
                ...
                try:
                    ...
                    print(bytesGiven)
                    self.addLabelReference(
                        trueByteValue, bytesGiven, labelsGiven, relativeLabel=True
                    )
                ...
                if trueByteValue[0] == "!":  # this is a label reference
                    ...
                    if trueByteValue.replace("\n", "")[-1] == "X":  # LDAzpgx
                        ...
                        bytesGiven.append(
                            int(trueByteValue.replace("$", "").split(",")[0], 16)
                        )
        ...
        for pointer, byteIn in enumerate(bytesGiven):
            ...
            if "ref" in str(byteIn):
                ...
                for pref in enumerate(labelsGiven):
                    # THIS CODE HERE IS SPECIFICALLY FOR RELATIVE LABELS
                    # if you want the normal labels go next if loop
                    # first check if it contains rel
                    if "REL!" in byteIn:
                        # next check if the thing that contains rel
                        # also contains a valid label
                        if str(pref[1]["name"] == byteIn.split("REL!")[1]):
                            newData = byteIn.split("REL!")[1].split("!")
                            pointInProgram, labelNameGiven = (
                                int(newData[0]),
                                newData[1],
                            )  # return correct types
                            # okay so we have the point in the program
                            # where the program counter is (probably)
                            # like BEQ !final is at 8 that is what
                            # pointInProgram is. so then we need to final where
                            # that fat little labelNameGiven is located and then
                            # compare it with the relative label. if it's
                            # bigger than relative label we will go and add that int
                            # as a signed boolean integer
                            # but if it's less than the relative label then
                            # we will use PROFESSOR NAUGHTONS beautiful 2 bit's
                            # compliment signed bit integer to set the correct position
                            # and then and only then hopefully the jump will go the correct
                            # amount of bytes
                            # okay okay
                            # so then first we need to find the relative label
                            for label in labelsGiven:
                                # A. check that the name is the same
                                # B. check that the location is not undefined
                                if (
                                    label["name"] == labelNameGiven
                                    and label["location"] != False
                                ):
                                    # huzzah!!! we found the pointinProgram
                                    # i'm terrible at writing comments and namespaces
                                    # but this is the labelGiven
                                    # at it's non relative address
                                    labelGivenNonRelativeAddress = label["location"]
                            # okay so now that we have both the current pc
                            # and the relative address,
                            # let's compare them at their points

                            if (
                                labelGivenNonRelativeAddress - pointInProgram
                            ) > 127 or (
                                labelGivenNonRelativeAddress - pointInProgram
                            ) < -128:
                                raise MemoryError(
                                    "ERR: Label out of range! Relative addressing cannot jump to the address"
                                )
                            if labelGivenNonRelativeAddress >= pointInProgram:
                                returnRelativeAddress = (
                                    labelGivenNonRelativeAddress - pointInProgram
                                )
                            elif labelGivenNonRelativeAddress <= pointInProgram:
                                offset = labelGivenNonRelativeAddress - pointInProgram
                                twosCompliment = (offset + (1 << 8)) % (
                                    1 << 8
                                )  # 8 is the bit width, this is a clear
                                # example i stole from the internet to get twos compliment
                                returnRelativeAddress = twosCompliment
                            # NOW STORE IT GOLLLLLLL
                            # this gets the value relatively to the correct offset.
                            # now time to program all the edge cases :|
                            bytesGiven[pointer] = returnRelativeAddress
                    elif (
                        str(pref[1]["name"]) == byteIn.replace("ref!", "")
                        and pref[1]["location"] != False
                    ):  # if this is assigned
                        ...
                        twoByteAdd = pref[1][
                            "location"
                        ]  # + globalFixPointerFromImmediateOffset
        ...
        for byte in bytesGiven:
            ...
            if "ref" in str(byte):
                print(
                    "ERR: unassigned label! -- "
                    + str(counterByteErrLabel)
                    + " "
                    + str(byte)
                )
                raise MemoryError
        ...
        for value in hex_list:
            ...
            for opcode, opcodevalues in self.optable.items():
                ...
                if (
                    opcodevalues["bytes"] == int(str(value), 16)
                    and noOpcodeNewLine == 0
                ):
                    # display the correct opcode and its value
                    lineToPrint = (
                        lineToPrint
                        + self.apple1AddressConversion(miniProgramCounter)
                        + " Instruction: "
                        + str(opcode + " (" + str(value) + ") - ")
                    )
                    miniProgramCounter += opcodevalues["space"]
                    noOpcodeNewLine = (
                        opcodevalues["space"] - 1
                    )  # add this to determine the newline
            ...
            if not foundOpCode:
                ...
                lineToPrint = (
                    lineToPrint
                    + " | "
                    + self.apple1AddressConversion(int(value, 16))
                    + " | "
                )
                if noOpcodeNewLine == 0:
                    lineToPrint = (
        ...
        open("debugProgram.txt", "w").write(lineToPrint)
        if debug:
            print(lineToPrint)
    ...
    def executeInstruction(self, opCodeToexecute, stepMode=False):
        ...
        if stepMode:
            ...
            open("debug.log", "a").write(
                ...
                + str(self.zeroFlag)
                + " I: "
                + str(self.interruptFlag)
                + " D: "
                + str(self.decimalFlag)
                + " B: "
                + str(self.breakFlag)
                + " V: "
                + str(self.overflowFlag)
                + " N: "
                + str(self.negFlag)
                + "\n"
            )
        ...
        if opInt == 0xE0:  # CPX immediate
            ...
            if self.x >= firstValueByte:  # if greater carry flag tru
                open("debug.log", "a").write(
                    "!!!COMPARE IS GREATER - carryflag on!!!!\n"
                )
                self.carryFlag = 1
            if self.x <= firstValueByte:  # if lesser carry flag false
                open("debug.log", "a").write(
                    "!!!COMPARE IS LESSER - carryflag off!!!!\n"
                )
                self.carryFlag = 0
            ...
            if self.negFlag == 1:
                open("debug.log", "a").write(
                    "!!!COMPARE IS NEGATIVE BITWISE - negflag on!!!!\n"
                )
            self.pc += 2
        if opInt == 0x60:  # RTS impl
            ...
            open("debug.log", "a").write(
                "Program Counter: Returning to address {} from subroutine\n".format(
                    finalByte
                )
            )
        ...
        if opInt == 0x9A:  # TXS
            self.pc += 1
            self.sp -= 1
            self.setB(self.sp, self.x)
        if opInt == 0xBA:  # TSX
            self.pc += 1
            self.x = self.getB(self.sp)
            print(self.apple1AddressConversion(self.sp))
        if opInt == 0xC9:  # CMP immediate
            if self.a == firstValueByte:
                self.zeroFlag = 1
                result = 0  # IMPORTANT FOR ZEROFLAG
                open("debug.log", "a").write("Zero Flag = 1\n")
            if self.a >= firstValueByte:
                open("debug.log", "a").write("Carry Flag = 1\n")
                self.carryFlag = 1
        ...
        if opInt == 0x90:  # CMP
            if self.carryFlag == 1:
                ...
                open("debug.log", "a").write(
                    "Program Counter: Carry flag true, jumped "
                    + str(firstValueByte)
                    + " bytes forward\n"
                )
        ...
        if opInt == 101:  # STA zpg
            ...
            result = self.a
        if opInt == 0x10:  # BPL rel
            if self.negFlag == 0:
                self.setProgramCounterValueFromRelativeAddress(firstValueByte)
        if opInt == 0x30:  # BMI rel
            if self.negFlag == 1:
                self.setProgramCounterValueFromRelativeAddress(firstValueByte)

        if opInt == 0xF0:  # BEQ rel
            # print("\n" + str(self.zeroFlag))
            # input("ya")
            if self.zeroFlag == 1:
                self.setProgramCounterValueFromRelativeAddress(firstValueByte)
                self.displayCommandAfter = False
                open("debug.log", "a").write(
                    "Program Counter: Zeroflag true, jumped by "
                    + str(firstValueByte)
                    + " \n"
                )
        ...
        if opInt == 0x20:  # JSR abs
            ...
            returnAddressForRT = (
                self.pc + 3
            )  # the expression space between the namespace and the two bit address
            ########
            # TO CLARIFY
            ...
            open("debug.log", "a").write(
                "Program Counter: Jumped to subroutine at $"
                + str(firstValueByte)
                + " with return address $"
                + str(returnAddressForRT)
                + " \n"
            )
    ...
    def start(self, debug=False):
        ...
        while True:
            try:
                if debug:
                    returnCode = self.executeInstruction(
                        memoryHex[self.pc], stepMode=True
                    )
                else:
